...
namespace libgp {
  ...
  public:
    ...
    /** Returns a pointer to the sampleset. */
    SampleSet * get_sampleset() {return sampleset;};
    ...
    void log_likelihood_gradient(Eigen::VectorXd & grad);

    Eigen::VectorXd log_likelihood_gradient();

    /** Returns a reference to the Cholesky decomposition of the Gram matrix */
    const Eigen::MatrixXd & getCholesky();
    
    Eigen::VectorXd get_alpha(){update_alpha();return alpha;}; // amarcovalle: added to have access to alpha

    Eigen::VectorXd get_k_star(){return k_star;}; // amarcovalle: added to have access to k_star
...
//    Eigen::LLT<Eigen::MatrixXd> solver;
    ...
    // Temporary objects to compute the log likelihood gradient.
    Eigen::VectorXd g;
    Eigen::MatrixXd W;
  };
}
