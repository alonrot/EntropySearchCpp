...
namespace libgp
{
  ...
  void CovSum::compute_dkdx(const Eigen::VectorXd & x, const Eigen::Ref<const Eigen::VectorXd> & kZx, const Eigen::Ref<const Eigen::MatrixXd> & Z, Eigen::Ref<Eigen::MatrixXd> dkZxdx){
	  
    // Compute kernel derivative of the first kernel:
    first->compute_dkdx(x, kZx, Z, dkZxdx);

    // Compute kernel derivative of the second kernel:
	  Eigen::MatrixXd dkZxdx_second = Eigen::MatrixXd::Zero(dkZxdx.rows(),dkZxdx.cols());
	  second->compute_dkdx(x, kZx, Z, dkZxdx_second);

    // Add them:
	  dkZxdx += dkZxdx_second;
  }
