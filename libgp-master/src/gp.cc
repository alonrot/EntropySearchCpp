...
namespace libgp {
  ...
  void GaussianProcess::log_likelihood_gradient(Eigen::VectorXd & grad) 
  {
    // TODO: Is this method numerically stable?
    compute();
    update_alpha();
    ...
    W = W.transpose() * W; // TODO: there is a faster way to to this - rank update or something
  ...
  {
    Eigen::VectorXd grad(cf->get_input_dim());
    log_likelihood_gradient(grad);
    return grad;
  }

  const Eigen::MatrixXd & GaussianProcess::getCholesky(){
    if (cf->loghyper_changed)
      compute();
    // TODO: it would be nice if we could return a view on the non-zero part here
    // size_t n = sampleset->size();
    // return L.topLeftCorner(n, n);
    return L;
  }
}
